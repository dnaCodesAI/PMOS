...
- **No stale data shown as current.** Pace status is never rendered from balance data older than 24 hours without a visible stale-data indicator and last-updated timestamp (AC-12). On fetch failure, the status is suppressed entirely (AC-11).
